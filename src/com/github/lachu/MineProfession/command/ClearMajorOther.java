	...
	public boolean execute(MineProfession mp, CommandSender sender, Command cmd, String[] args) {
		...
		if((target=mp.getServer().getPlayer(args[1]))!=null){
			if(mp.data.clearMajor(args[1])){
				...
				target.sendMessage(ChatColor.GREEN+"Your major and minor profession are cleared by "+sender.getName()+".");
			}else{
				sender.sendMessage(ChatColor.YELLOW+args[1]+" do not have a major profession.");
			}
		}else{
